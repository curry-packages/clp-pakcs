...
'CLP.R.prim_CLPR_plus'(Y,X,R) :- {R = X+Y}.

'CLP.R.prim_CLPR_minus'(Y,X,R) :- {R = X-Y}.

'CLP.R.prim_CLPR_times'(Y,X,R) :- {R = X*Y}.

'CLP.R.prim_CLPR_div'(Y,X,R) :- {R = X/Y}.

'CLP.R.prim_CLPR_le'(Y,X,'Prelude.True') :- {X < Y}.

'CLP.R.prim_CLPR_ge'(Y,X,'Prelude.True') :- {X > Y}.

'CLP.R.prim_CLPR_leq'(Y,X,'Prelude.True') :- {X =< Y}.

'CLP.R.prim_CLPR_geq'(Y,X,'Prelude.True') :- {X >= Y}.

% transform an integer into a float:
'CLP.R.prim_CLPR_i2f'(X,R) :- R is X*1.0.
